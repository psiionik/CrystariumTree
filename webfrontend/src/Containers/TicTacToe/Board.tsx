            ...
            <div className={styles.board}>
                {this.props.board.map((list, row_index) => {
                    return list.map((item, col_index) => {
                        return <Square key={(3 * row_index)+ col_index} row_index={row_index} col_index={col_index} item={item} clickFunction={this.props.clickFunction} playing={this.props.playing} />
                    })
                })}
            </div>
